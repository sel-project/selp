				...
				if(server.name != "") {
					...
					if(!exists(exe) || force) {
						...
						if(server.type == "node" || server.type == "full") {
							string location = server.location ~ (full ? "node" ~ dirSeparator : "");
							if(exists(location ~ __EXECUTABLE__)) remove(location ~ __EXECUTABLE__);
							wait(spawnShell("cd " ~ location ~ " && rdmd " ~ args.join(" ") ~ " init.d"));
							string[] nargs = args.dup;
							nargs ~= "-J" ~ server.location ~ ".hidden"; // for settings generated by init.d
							nargs ~= "-I" ~ server.location ~ "plugins"; // for plugins (managed by init.d)
							if(exists(server.location ~ ".hidden" ~ dirSeparator ~ "plugintemp")) nargs ~= "-I" ~ cast(string)read(server.location ~ ".hidden" ~ dirSeparator ~ "plugintemp"); // for compressed plugin (managed by init.d)
							wait(spawnShell("cd " ~ location ~ " && rdmd --build-only " ~ nargs.join(" ") ~ " main.d"));
						}
						if(server.type == "hub" || server.type == "full") {
							string location = server.location ~ (full ? "hub" ~ dirSeparator : "");
							if(exists(location ~ __EXECUTABLE__)) remove(location ~ __EXECUTABLE__);
			...
			if(args.length > 1) {
				...
				if(server.name != "") {
					if(server.type == "node") {
						...
						void connect() {
							wait(spawnShell("cd " ~ server.location ~ " && ." ~ dirSeparator ~ "main " ~ name ~ " " ~ ip ~ " " ~ to!string(port) ~ " " ~ to!string(main)));
						}
						wait(spawnShell(launch ~ " build " ~ server.name ~ " -force=false"));
						immutable pdir = server.location ~ ".hidden" ~ dirSeparator ~ "rebuild";
						...
						if(type == "hub" || type == "node" || type == "full") {
							...
							if(vers != "none") {
								...
								version(Windows) {
									...
									foreach(string s ; ["common", "res", "node", "hub", "sel", "init.d", "main.d"]) {
										if(exists(path ~ s)) setAttributes(path ~ s, FILE_ATTRIBUTE_HIDDEN);
									}
								}
							}
				...
				if(server.name != "") {
					...
					if(server.type == "hub") {
						...
						do {
							...
							new Thread({ wait(spawnShell("cd " ~ server.location ~ "hub && ." ~ dirSeparator ~ "main")); }).start();
							executeShell("cd " ~ server.location ~ "node && ." ~ dirSeparator ~ "main");
