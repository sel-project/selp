				...
				if(server.name != "") {
					...
					if(!exists(exe) || force) {
						...
						if(server.type == "node" || server.type == "full") {
							...
							if(exists(server.location ~ ".hidden" ~ dirSeparator ~ "plugintemp")) nargs ~= "-I" ~ tempDir() ~ dirSeparator ~ "sel" ~ dirSeparator ~ cast(string)read(server.location ~ ".hidden" ~ dirSeparator ~ "plugintemp"); // for compressed plugins (managed by init.d)
